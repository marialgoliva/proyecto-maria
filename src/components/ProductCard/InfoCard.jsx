...
function InfoCard({product, colors, tallas}) {
  ...
  return (
    <div className="rounded p-4 bg-light d-flex row justify-content-between">
      <h4 className="fw-normal">{nombre} <span className="text-secondary fs-4 fw-normal text-capitalize">{color}</span></h4>
      <div className="d-flex row justify-content-end">
        ...
        <div className="d-flex row gap-3">
            <div className="d-flex justify-content-end gap-2">
                {tallas.map((talla) => {
                    return <button type="button" className="btn btn-outline-secondary">{talla}</button>
                }
                    
                )}
            </div>
                <Link href="" className="text-decoration-none text-secondary text-end"><p>¿Qué talla elijo?</p></Link>
        </div>
        <div className="d-flex row justify-content-end">
      ...
      <button type="button" className="btn btn-secondary mt-5 w-75" style={{height: '40px'}}>Añadir al carrito</button>
    </div>
  );
}

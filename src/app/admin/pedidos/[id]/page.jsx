...
async function getProductos(id) {
  const {data} = await axios.get(`http://localhost:3000/api/producto-pedido/${id}`);
  return data;
}
...
async function obtenerProductosDelPedido(idPedido) {
  const respuesta = await fetch(`http://localhost:3000/api/producto-pedido/${idPedido}/productos`);
  if (!respuesta.ok) {
...
async function PaginaPedido({params}) {
    ...
    <div className="d-flex flex-column m-4">
      ...
      <ul className="fs-5 list-group">
        <li className="list-group-item">Cliente: {pedido.idCliente} - {nombre} {apellidos}</li>
        <li className="list-group-item">Estado: {pedido.estado}</li>
        <li className="list-group-item">Fecha de creación: {dayjs(pedido.fechaPedido).locale('es').format('DD/MM/YYYY')}</li>
        <li className="list-group-item"><strong>Fecha de entrega: {dayjs(pedido.fechaEntrega).locale('es').format('DD/MM/YYYY')}</strong></li>
        <li className="list-group-item">Importe total: {pedido.importeTotal} €</li>
